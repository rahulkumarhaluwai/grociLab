  ...
  useEffect(()=>{
    ...
    api.get(`/products/${id}`).then(({data})=>{
      ...
      return api.get(`/products?category=${data.product.category}`)
        ...
        setRelatedProducts(data.products.filter((p: Product)=>p._id !== id))
    }).catch(()=>
      navigate("/products")).finally(()=> setLoading(false))
  },[id, navigate])
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            <div className="relative flex-center p-8 md:p-12 min-h-80 md:min-h-120">
              ...
              {product.isOrganic && (
                <span className="flex items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-app-green text-white rounded-full"><LeafIcon className="w-3 h-3"/>
                  Organic</span>
              )}{ product.discount > 0 &&(
            ...
            <div className="p-6 md:p-10 flex flex-col justify-center">
              ...
              <div className="flex items-center gap-3">
                ...
                }} disabled={product.stock === 0} className={`flex-1 py-3 font-semibold rounded-xl transition-colors flex-center gap-2 
                  disabled:opacity-50 disabled:cursor-not-allowed active:scale[0.98] ${inCart ? 
                  "bg-app-cream text-app-green border border-app-green" : "bg-app-orange text-white hover:bg-app-orange-dark"}`}>
